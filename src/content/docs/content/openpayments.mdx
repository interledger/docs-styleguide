...
Use the `:::tip` aside and include a brief description of the guide's purpose. Don't repeat the page title.

### Overview

Include some general information about the guide that helps to set up the subsequent Scenario section.

For example, in _Set up recurring payments with a fixed incoming amount_, we use a "subscription payment" as a general example of a recurring transfer of money of the same amount. Then, the scenario section uses "streaming service" as a specific type recurring subscription.
...
:::note
This was a style that came about organically, not one that was discussed. If you find guides that don't follow this format, update them. Not a high priority.
:::
...
- Each step should have some type of explanation about its purpose.
...
- Pay attention to timestamp values (for example, `createdAt`). They should incrementally increase as the steps progress.
