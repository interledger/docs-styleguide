...
:::note
We use an [in-house Mermaid component](../../shared/mermaid) that calls the Mermaid JavaScript API directly. Our component may not support every notation that’s supported by Mermaid itself (e.g., diagonal lines). Additionally, some style decisions may deviate from industry norms due to Mermaid specifically and/or our component’s capabilities and limitations.
:::
