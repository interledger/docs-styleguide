...
```md
![An illustration of planets and stars featuring the word “astro”](/img/image.png)
```

We standardise our image location for every project to be in the `/public/img` folder. You may want to further organise that folder if the project has a lot of images.
...
:::caution
This component requires an extra dependency on [remark-mermaidjs](https://github.com/remcohaszing/remark-mermaidjs) and thus is not enabled by default on all projects. Please ping the person responsible for documentation architecture if your project doesn't render mermaid diagrams properly.
:::
